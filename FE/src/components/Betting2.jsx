...
const AdminSuggestions2 = () => {
  ...
  const toggleSuggestion = () => {
    ...
    socket.emit("toggleSuggestion2", newState);
  };

  const handleSetBet = (type, value) => {
    socket.emit("setBetFromSuggestion2", { type, value }, (response) => {
      if (response.success) {
  ...
  useEffect(() => {
    socket.on("suggestions2", (data) => {
        setSuggestions(data.suggestions);
      });

    return () => socket.off("suggestions2");
  }, []);
...
export default AdminSuggestions2;
