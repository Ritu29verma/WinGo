...
const BetsData = () => {
  ...
  const totalPages = Math.ceil(betsData.length / postsPerPage);
const currentBets = betsData.slice((currentPage - 1) * postsPerPage, currentPage * postsPerPage);
    ...
    <Loader />
        ...
        <div className={`bg-gray-800 rounded-lg p-4 mx-auto ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          {currentBets.length === 0 ? (
            ...
            <div className="space-y-4">
              {currentBets.map((result) => (
                <div key={result._id} className="bg-slate-800 rounded-lg shadow-md p-4 flex justify-between">
                 <div>
                 ...
                 <p className="text-gray-400 text-md">{result.periodNumber}</p>
                  ...
                  <p className={result.winLoss > 0 ? "text-green-400" : "text-red"}>
